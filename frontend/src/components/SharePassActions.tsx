...
export function SharePassActions({
  ...
  const [emailModal, setEmailModal] =
    useState<EmailModalState>(INITIAL_EMAIL_MODAL);
  ...
  const menuItemCount =
    1 + (enableEmailShare ? 1 : 0) + (canNativeShare ? 1 : 0);
  ...
  const updateDropdownPosition = useCallback(() => {
    ...
    const dropdownHeight =
      dropdownRef.current?.offsetHeight ?? menuItemCount * 44 + 16;
    const gap = 6;
  ...
  return (
    <>
      ...
      >
        <div
          className={`flex gap-2 ${ticketLayout ? 'w-full' : compact ? 'flex-wrap' : 'flex-wrap w-full'}`}
        >
          {showQrLink && (
            <Link
          ...
          <div
            className={`share-menu__trigger-wrap ${ticketLayout ? 'flex-1 min-w-0' : ''}`}
          >
            <button
              ref={triggerRef}
              type="button"
        ...
        >
          ...
          >
            ...
            {emailModal.result.phase === 'sending' && (
              <div
                className="share-modal__status share-modal__status--pending"
                role="status"
                aria-live="polite"
              >
                <Loader2 className="w-5 h-5 shrink-0 animate-spin" />
                <div>
                  <div className="share-modal__status-title">
                    {labels.buttons.sendEmailSending}
                  </div>
            ...
            {emailModal.result.phase === 'success' && (
              <div
                className="share-modal__status share-modal__status--success"
                role="status"
                aria-live="polite"
              >
                ...
                <div className="min-w-0">
                  ...
                  <div className="share-modal__status-meta">
                    ...
                    {emailModal.result.email
                      ? ` · ${emailModal.result.email}`
                      : ''}
                  </div>
                  <p className="share-modal__status-hint">
                    {labels.buttons.sendEmailSuccessHint}
                  </p>
                </div>
              </div>
            )}
            ...
            {emailModal.result.phase === 'error' && (
              <div
                className="share-modal__status share-modal__status--error"
                role="alert"
              >
                <AlertCircle className="w-5 h-5 shrink-0" />
                <div className="min-w-0">
                  <div className="share-modal__status-title">
                    {labels.buttons.sendEmailError}
                  </div>
                  <div className="share-modal__status-meta">
                    {emailModal.result.status
                      ? `HTTP ${emailModal.result.status} · `
                      : ''}
                    {emailModal.result.message}
                  </div>
                </div>
