  ...
  const load = () => {
    ...
    return api.admin
      ...
      .then((data) => {
        ...
        setMeta(`${data.meta.title} · ${data.meta.count} маршрутов · schema ${data.meta.schemaVersion}`);
  ...
  return (
    <AdminLayout title="API Mstyle">
      ...
      <p className="text-[var(--muted)] -mt-2 mb-6">
        {meta || '58 маршрутов из эндпоинты.md. Путь password-verify как в коде, не password:verify.'}
      </p>
      {loading ? (
                ...
                <li key={item.id}>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className={`font-mono text-[10px] px-1.5 py-0.5 rounded ${methodClass(item.method)}`}>
                        {item.method}
                      </span>
            ...
            <section className="card p-5 space-y-4 min-w-0">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className={`font-mono text-xs px-2 py-1 rounded ${methodClass(active.method)}`}>
                    {active.method}
                  </span>
                  <h2 className="font-semibold">
                    {active.id} · {active.title}
                  </h2>
                  <span className="text-xs text-[var(--muted)]">{active.milestone}</span>
                </div>
