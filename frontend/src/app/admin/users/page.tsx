...
const EMPTY_FORM: CreateUserData = {
  email: '', password: '', fullName: '', role: 'resident', phone: '', apartment: '', building: '',
};
...
export default function AdminUsersPage() {
  ...
  const openEdit = (u: AdminUser) => {
    setEditId(u.id);
    setForm({ email: u.email, password: '', fullName: u.fullName, role: u.role, phone: u.phone || '', apartment: u.apartment || '', building: u.building || '' });
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card p-5 mb-6 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="label">{editId ? 'Новый пароль' : 'Пароль *'}</label>
              <input className="input" type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required={!editId} minLength={6} />
            </div>
            <div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-[var(--muted)]">
            <tr>
              ...
              <th className="text-left p-3 font-medium hidden sm:table-cell">Кв.</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="p-8 text-center text-[var(--muted)]">Пользователи не найдены</td></tr>
            ) : users.map((u) => (
              <tr key={u.id} className="border-t border-[var(--border)] hover:bg-slate-50">
                <td className="p-3 font-medium">{u.fullName}</td>
                <td className="p-3 hidden md:table-cell text-[var(--muted)]">{u.email}</td>
                <td className="p-3">{ROLE_LABELS[u.role]}</td>
                <td className="p-3 hidden sm:table-cell">{u.apartment || '—'}</td>
                <td className="p-3">
                ...
                <td className="p-3">
                  <button className="p-1 hover:text-[var(--primary)]" onClick={() => openEdit(u)}><Pencil className="w-4 h-4" /></button>
                </td>
              </tr>
            ))}
          </tbody>
