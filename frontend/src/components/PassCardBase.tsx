...
export function PassCardBase({
  ...
  const useTimeline =
    (showTimeline ?? !isCompact) && passShowsVisitTimeline(pass);
    ...
    >
      <div className="flex items-stretch min-w-0 w-full">
        ...
        <div className="pass-card__body">
          {overdueKind && (
            <div
              className={`px-4 py-2 theme-alert-subtle border-b text-xs sm:text-sm pass-card__alert ${isCompact ? 'px-3' : ''}`}
            >
          ...
          <div
            className={`border-b border-[var(--border)] bg-gradient-surface ${isCompact ? 'px-3 pt-3 pb-2' : 'px-4 pt-4 pb-3'}`}
          >
            <div className="flex items-start gap-3 min-w-0">
              {pass.companyLogo ? (
                <div
                  className={`shrink-0 overflow-hidden ${isCompact ? 'w-12 h-12' : 'w-14 h-14'}`}
                >
                ...
                <div
                  className={`${getPassIconTileClass(pass.status, stillInside)} shadow-sm shrink-0 ${isCompact ? 'w-10 h-10' : 'w-12 h-12'}`}
                >
            ...
            {(pass.businessCenterName || pass.companyName || pass.office) && (
              <div
                className={`pass-card__chips mt-2 text-[var(--muted)] ${isCompact ? 'text-xs' : 'text-sm'}`}
              >
                <span
          ...
          {useTimeline && (
            <div
              className={`pass-card__timeline ${isCompact ? 'px-2 sm:px-3 py-3' : 'px-3 sm:px-5 py-4'} ${isTerminal ? 'bg-[var(--surface-muted)]' : 'bg-[var(--surface)]'}`}
            >
              <PassVisitTimeline
                pass={pass}
                labels={labels}
          ...
          <div
            className={`border-t border-[var(--border)] bg-[var(--surface-muted)] ${isCompact ? 'px-3 py-2' : 'px-4 py-3'}`}
          >
            <div
              className={`pass-card__chips ${isCompact ? 'text-xs' : 'text-sm'}`}
            >
              <span
              ...
              >
                ...
                <span className="pass-card__visit-date font-semibold">
                  {visitDateLabel}
                </span>
                {visitWindow && (
              ...
              {pass.vehiclePlate && (
                <span
                  className="pass-card__chip font-mono font-semibold"
              ...
              {!showCreator &&
                ...
                !isCompact && (
                  <span
                    className="pass-card__chip text-[var(--muted)] text-xs"
                    title={[pass.creatorName, pass.creatorCompany]
                      .filter(Boolean)
                      .join(' · ')}
                  >
