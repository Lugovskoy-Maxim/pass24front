...
export default function AdminSettingsPage() {
  ...
  return (
    <AdminLayout title="Настройки системы">
      <p className="text-[var(--muted)] -mt-4 mb-6">Бизнес-правила и параметры пропускного режима</p>

      <form onSubmit={handleSubmit} className="card p-6 max-w-xl space-y-5">
        <div>
          <label className="label">Название ЖК</label>
          <input className="input" value={settings.complex_name} onChange={(e) => setSettings({ ...settings, complex_name: e.target.value })} />
        </div>

        <div>
          <label className="label">Лимит пропусков на жителя в день</label>
        ...
        <label className="flex items-center gap-2 text-sm">
          <input
            type="checkbox"
            checked={settings.auto_approve_delivery === 'true'}
            onChange={(e) => setSettings({ ...settings, auto_approve_delivery: e.target.checked ? 'true' : 'false' })}
          />
