"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PermissionsGuard = void 0;
const common_1 = require("@nestjs/common");
const core_1 = require("@nestjs/core");
const access_config_service_1 = require("../access/access-config.service");
const permissions_decorator_1 = require("./permissions.decorator");
let PermissionsGuard = class PermissionsGuard {
    reflector;
    accessConfigService;
    constructor(reflector, accessConfigService) {
        this.reflector = reflector;
        this.accessConfigService = accessConfigService;
    }
    async canActivate(context) {
        const anyRequired = this.mergePermissions(context, permissions_decorator_1.PERMISSIONS_KEY);
        const allRequired = this.mergePermissions(context, permissions_decorator_1.PERMISSIONS_ALL_KEY);
        if (!anyRequired.length && !allRequired.length)
            return true;
        const { user } = context.switchToHttp().getRequest();
        if (!user?.role)
            throw new common_1.ForbiddenException('Нет доступа');
        const permissions = await this.accessConfigService.getPermissionsForRole(user.role);
        if (allRequired.length && !allRequired.every((p) => permissions.includes(p))) {
            throw new common_1.ForbiddenException('Недостаточно прав');
        }
        if (anyRequired.length && !anyRequired.some((p) => permissions.includes(p))) {
            throw new common_1.ForbiddenException('Недостаточно прав');
        }
        return true;
    }
    mergePermissions(context, key) {
        const handler = this.reflector.get(key, context.getHandler()) || [];
        const cls = this.reflector.get(key, context.getClass()) || [];
        return [...new Set([...cls, ...handler])];
    }
};
exports.PermissionsGuard = PermissionsGuard;
exports.PermissionsGuard = PermissionsGuard = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [core_1.Reflector,
        access_config_service_1.AccessConfigService])
], PermissionsGuard);
//# sourceMappingURL=permissions.guard.js.map