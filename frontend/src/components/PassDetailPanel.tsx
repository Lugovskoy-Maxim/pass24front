...
export function PassDetailPanel({ pass, labels: labelsProp, showCreator, actions, children }: PassDetailPanelProps) {
  ...
  return (
    <div className="card overflow-hidden border border-[var(--border)] shadow-sm">
      {overdueKind && (
        <div className="px-4 py-2 bg-amber-50 border-b border-amber-200 text-amber-900 text-xs">
          {getOverdueCardMessage(overdueKind, pass, labels)}
        </div>
      )}

      <div className="px-4 py-4 bg-gradient-to-br from-[var(--primary)]/5 to-white border-b border-[var(--border)]">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="text-lg font-bold leading-tight break-words">{pass.visitorName}</h3>
            <p className="font-mono text-sm text-[var(--primary)] font-semibold mt-1">{pass.passNumber}</p>
          </div>
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          <span className="inline-flex items-center gap-1 text-xs bg-white border border-[var(--border)] rounded-md px-2 py-1">
          ...
          <Link
            ...
            target="_blank"
            className="inline-flex items-center gap-1 text-xs bg-white border border-[var(--border)] rounded-md px-2 py-1 text-[var(--primary)] hover:bg-[var(--surface-muted)]"
          >
      ...
      <div className={`px-4 py-4 border-b border-[var(--border)] ${isTerminal ? 'bg-[var(--surface-muted)]' : 'bg-white'}`}>
        <p className="text-[10px] uppercase tracking-wide text-[var(--muted)] mb-3">{labels.passes.detailTimeline}</p>
        <PassVisitTimeline pass={pass} labels={labels} />
      </div>
      ...
      <dl className="px-4 py-2 divide-y divide-[var(--border)]">
        ...
        <DetailRow label={labels.card.phone} value={pass.visitorPhone && (
          <a href={`tel:${pass.visitorPhone}`} className="text-[var(--primary)] hover:underline">{pass.visitorPhone}</a>
        )} />
        <DetailRow
            ...
            <dd className="text-sm space-y-1.5">
              {pass.creatorName && (
                <div className="flex items-start gap-2 font-medium">
                  <User className="w-4 h-4 text-[var(--primary)] shrink-0 mt-0.5" />
              ...
              {pass.creatorPhone && (
                <div className="flex items-center gap-2">
                  ...
                  <a href={`tel:${pass.creatorPhone}`} className="text-[var(--primary)] hover:underline">
                    {pass.creatorPhone}
                  </a>
                </div>
              )}
        ...
        {pass.rejectionReason && (
          <div className="py-2">
            <dt className="text-[10px] uppercase tracking-wide text-red-600 mb-1">{labels.card.rejectionReason}</dt>
            <dd className="text-sm text-red-600">{pass.rejectionReason}</dd>
          </div>
        )}
      </dl>
