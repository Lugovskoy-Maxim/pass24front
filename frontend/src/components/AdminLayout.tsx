...
export function AdminLayout({
  ...
  useEffect(() => {
    try {
      setCompactNav(
        localStorage.getItem('pass24-admin-nav-compact') !== 'false',
      );
    } catch {
    ...
    return api.admin
      ...
      .then(({ requests, count }) => {
        setRegistrationPending(
          typeof count === 'number' ? count : requests.length,
        );
      })
      .catch(() => undefined);
  }, [user]);
  ...
  useAutoRefresh(() => loadPending(), {
    enabled: hasPermission(user, 'admin.users'),
  });
  ...
  return (
    <ProtectedLayout permissions={['admin.panel']} wide>
      ...
      >
        ...
        >
          <div
            className={`card lg:sticky lg:top-20 ${compactNav ? 'p-2 lg:py-3' : 'p-3 lg:p-4'}`}
          >
            <div
              className={`hidden lg:flex items-center gap-2 mb-3 pb-3 border-b border-[var(--border)] ${
                compactNav ? 'justify-center' : ''
              }`}
            >
              <Shield className="w-5 h-5 text-[var(--primary)]" />
              <span
                className={`font-semibold text-sm ${compactNav ? 'lg:sr-only' : ''}`}
              >
                Администрирование
              </span>
            </div>
            ...
            <div className="hidden lg:block mt-3 pt-3 border-t border-[var(--border)]">
              ...
              >
                ...
                <span className={compactNav ? 'sr-only' : ''}>
                  К приложению
                </span>
                {compactNav && (
                  <span
