...
describe('Канонические данные человека для Mstyle', () => {
  ...
  test('PF-07 не требует passport.fullName для физического лица', () => {
    expect(requiredResidentFields('individual')).toEqual(['individual.birthDate']);
    expect(() =>
      mergeResidentValuesPreservingLegacy(
        {},
        {
    ...
    const result = safeIdentity(
      {
        ...
        name: { lastName: 'Сотрудников', firstName: 'Сергей', middleName: null },
  ...
  test('без явного запроса форма ответа для старого Mstyle не меняется', () => {
    const identity = safeIdentity({
      subject: 'usr_01', identityStatus: 'active', authVersion: 1, revision: 1,
      displayName: 'Name', name: {}, birthDate: '1990-01-01',
    } as any);
    const profile = safeProfile({
      profileId: 'prf_01', type: 'company', status: 'active', label: 'Label',
      companyName: 'Company', revision: 1, memberPolicy: {}, officeIds: [],
      membershipSetRevision: 1, assignmentSetRevision: 1,
    } as any);
