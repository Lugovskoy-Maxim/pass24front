...
function HistoryLink({
  ...
  return (
    ...
    >
      ...
      {hasMatches && (
        <span
          className="pass-detail__history-count"
          aria-label={formatVisitCount(count)}
        >
          {count}
        </span>
      )}
...
function formatPassport(pass: Pass) {
  if (!pass.visitorPassportSeries && !pass.visitorPassportNumber)
    return undefined;
  const base = [pass.visitorPassportSeries, pass.visitorPassportNumber]
    .filter(Boolean)
    .join(' ');
  return pass.visitorPassportIssuedBy
    ? `${base} · ${pass.visitorPassportIssuedBy}`
    : base;
}
...
export function PassDetailPanel({
  ...
  const canSeeHistory =
    canViewAllPasses(user) ||
    canUseReception(user) ||
    hasPermission(user, 'admin.panel');
  const canEditPassport =
    canUseReception(user) ||
    hasPermission(user, 'passes.approve') ||
    hasPermission(user, 'admin.panel');

  const [visitorVisitCount, setVisitorVisitCount] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (hasVisitorIdentity) {
      api
        ...
        .then(({ total }) => {
          if (!cancelled)
            setVisitorVisitCount(typeof total === 'number' ? total : 0);
        })
        .catch(() => {
    ...
    if (pass.visitorPhone?.trim()) {
      api
        ...
        .then(({ total }) => {
          if (!cancelled)
            setPhoneVisitCount(typeof total === 'number' ? total : 0);
        })
        .catch(() => {
  ...
  const returningVisitor =
    typeof visitorVisitCount === 'number' && visitorVisitCount > 1;

  return (
    <div
      className={`${getPassCardShellClass({ overdue: stillInside })} pass-detail min-w-0 max-w-full overflow-visible`}
    >
        ...
        <div className="pass-card__body min-w-0">
          {overdueKind && (
            <div className="px-4 py-2.5 theme-alert-subtle border-b text-xs pass-card__alert">
              <span className="pass-card__alert-msg">
          ...
          <div className="px-4 pt-4 pb-3 bg-gradient-surface border-b border-[var(--border)]">
            <div className="flex items-start gap-3 min-w-0">
              <div
                className={`shrink-0 ${getPassIconTileClass(pass.status, stillInside)} w-11 h-11`}
              >
              ...
              <div className="flex-1 min-w-0">
                <h3
                  className="pass-card__title text-lg leading-snug"
                ...
                <div className="flex items-center gap-0.5 mt-2 min-w-0">
                  ...
                  <CopyButton
                    value={pass.passNumber}
                    title={labels.buttons.copyNumber}
                  />
                </div>
                ...
                {canSeeHistory && (
                  <div className="mt-1.5 flex flex-wrap items-center gap-2">
                    <HistoryLink
                      ...
                      title={
                        typeof visitorVisitCount === 'number' &&
                        visitorVisitCount > 0
                    ...
                    {returningVisitor && (
                      <span
                        className="pass-detail__return-badge"
                        title="Гость уже был в БЦ"
                      >
                        повторный визит
                      </span>
                    )}
          ...
          {passShowsVisitTimeline(pass) && (
            <div
              className={`pass-card__timeline px-4 py-3.5 border-b border-[var(--border)] ${isTerminal ? 'bg-[var(--surface-muted)]' : 'bg-[var(--surface)]'}`}
            >
          ...
          <dl className="pass-detail__list px-4 py-1">
            ...
            {passport && (
              <InfoRow icon={IdCard} label="Паспорт" value={passport} />
            )}
              ...
              (pass.creatorName ||
                ...
                <div className="pass-detail__row pass-detail__row--block">
                  ...
                  <dd className="pass-detail__value pass-detail__value--stack">
                    ...
                    {pass.creatorPhone && (
                      <a
                        href={`tel:${pass.creatorPhone}`}
                        className="inline-flex items-center gap-1.5 text-link hover:underline text-sm"
                      >
            ...
            {pass.comment && (
              <div className="pass-detail__row pass-detail__row--block">
                ...
                <dd className="pass-detail__note">{pass.comment}</dd>
              </div>
            )}
